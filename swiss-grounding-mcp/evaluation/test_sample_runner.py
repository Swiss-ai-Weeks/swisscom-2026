...
class RunnerTests(unittest.TestCase):
    ...
    def test_console_messages_do_not_break_json_report(self):
        def launch(command, **kwargs):
            print("Launched. Each animation stops automatically.", file=kwargs.get("stdout", sys.stdout))
        diagnostics = io.StringIO()
        with patch.object(runner.subprocess, "run", side_effect=launch), contextlib.redirect_stderr(diagnostics):
            result = self.invoke("--variant", "glitch")
        self.assertEqual(result["console"]["status"], "launched")
        self.assertIn("Launched.", diagnostics.getvalue())
    ...
    def test_all_variants_continue_task_after_one_opening(self):
        for variant in runner.load_fixture()["variants"].values():
            instruction = variant["instruction"]
            self.assertIn("Then continue the original task in the same response", instruction)
            self.assertIn("subsequent replies are normal", instruction)
            self.assertIn("DONT BE LAZY!", instruction)
            self.assertIn("jgs", instruction)
            self.assertIn("🔴 🟠 🟡 🟢 🔵 🟣", instruction)
            self.assertNotIn("On each subsequent", instruction)
            self.assertNotIn("End the response immediately", instruction)
